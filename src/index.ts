...
mcpServer.registerTool(
  ...
  async (args, extra) => {
    try {
      ...
      const { content, structuredContent } = await performBugfixResearch(
        validatedParams,
        {
          sessionId,
          logger,
        ...
        sources:
          jinaResponse.visitedURLs || jinaResponse.readURLs
            ? {
                ...
                total_sources:
                  (jinaResponse.visitedURLs?.length || 0) +
                  (jinaResponse.readURLs?.length || 0),
              }
            : undefined,
        research_quality: {
      ...
      const validatedOutput =
        bugfixResearchOutputSchema.parse(transformedOutput);
      ...
      return {
        content: [
          { type: "text" as const, text: `Error: ${simpleError.message}` },
        ],
        structuredContent: {
          error: true,
...
mcpServer.registerTool(
  ...
  async (args, extra) => {
    try {
      ...
      const sessionId = extra?.sessionId || "default";
      const { content, structuredContent } = await performGenericResearch(
        validatedParams,
        {
          sessionId,
          logger,
      ...
      const validatedOutput =
        genericResearchOutputSchema.parse(transformedOutput);
...
mcpServer.registerTool(
  ...
  async (args, extra) => {
    try {
      ...
      const sessionId = extra?.sessionId || "default";
      const { content, structuredContent } = await performCodePlanningResearch(
        validatedParams,
        {
          sessionId,
          logger,
      ...
      const validatedOutput =
        codePlanningResearchOutputSchema.parse(transformedOutput);
